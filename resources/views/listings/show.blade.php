...
<div class="listing-detail">
    <h1>{{ $listing->title }}</h1>
    <p class="meta">{{ $listing->category->name }} &bull; {{ $listing->location }} &bull; Posted by {{ $listing->user->name }}</p>
        ...
        @if(auth()->id() === $listing->user_id || auth()->user()->isAdmin())
            <div class="listing-actions">
                ...
                <form action="{{ route('listings.destroy', $listing) }}" method="POST" style="display:inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn-red" onclick="return confirm('Are you sure?')">{{ __('messages.delete') }}</button>
                </form>
            </div>
        @endif
    @endauth
...
// then drop an embedded map iframe in. this is the optional map feature.
    ...
    fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(location)}&format=json`)
        ...
        .then(data => {
            ...
            if (data.length > 0) {
                ...
                mapDiv.innerHTML = `<iframe width="100%" height="300" src="https://www.openstreetmap.org/export/embed.html?bbox=${lon-0.01},${lat-0.01},${lon+0.01},${lat+0.01}&marker=${lat},${lon}"></iframe>`;
            }
        });
}
</script>
